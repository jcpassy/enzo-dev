...
int SetDefaultGlobalValues(TopGridData &MetaData)
{
  ...
  GravitySolverType           = GRAVITY_SOLVER_FAST; // multigrid
  APMAddParentContribution    = TRUE;                // on
  TimeSteppingRefinementCondition = 0;               // Extra condition dt(l+1) <= dt(l)/refinement
  DepositAlsoParentGridAndSiblingsParticles = 0;     // Deposit also particles from parent and its siblings
