        ...
        for (i = 0; i < GridDimension[0]; i++) {
          ...
          if (r < max(SphereRadius, CellWidth[0][0])) {
            if (SphereType == 0)
              ...
              ENZO_VFAIL("Error in Grid_TestGravitySphereInitializeGrid.C: wrong type (%"ISYM") chosen.\n", SphereType);
          }
          else {
            density = SphereExteriorDensity;
 
          }
          BaryonField[0][k*GridDimension[0]*GridDimension[1] + j*GridDimension[0] + i] = density;
	}
