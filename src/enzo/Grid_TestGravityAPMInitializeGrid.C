/***********************************************************************
/
/  GRID CLASS (INITIALIZE THE GRID FOR A GRAVITY TEST) - APM version
/
...
int grid::TestGravityAPMInitializeGrid(float CentralDensity,
                                       ...
                                       int UseBaryons)
{
  ...
  if (NumberOfNewParticles > 0) {
    ...
    for (i = 0; i < NumberOfParticles; i++) {
      ...
      for (dim = 0; dim < GridRank; dim++)
        ParticlePosition[dim][i] += 0.5*(DomainLeftEdge[dim] + DomainRightEdge[dim]);
    ...
    if (!UseBaryons) {
      ...
      ParticlePosition[0][0] = CentralParticlePositionX;
      ParticlePosition[1][0] = CentralParticlePositionY;
      ParticlePosition[2][0] = CentralParticlePositionZ;
