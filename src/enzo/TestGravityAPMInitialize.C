/***********************************************************************
/
/  INITIALIZE A GRAVITY TEST - APM version
/
/  written by: Greg Bryan
/  date:       April, 1995
/  modified1: Jean-Claude Passy, May 2018
/
/  PURPOSE:
...
int TestGravityAPMInitialize(FILE *fptr, FILE *Outfptr, HierarchyEntry &TopGrid,
                             TopGridData &MetaData)
{
  ...
  float TestGravityCentralParticlePosition[MAX_DIMENSION];
  for (dim = 0; dim < MAX_DIMENSION; dim++)
    TestGravityCentralParticlePosition[dim] = 0.5 * (DomainLeftEdge[dim] + DomainRightEdge[dim]);
  ...
  while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL)
  {
    ...
    ret += sscanf(line, "TestGravityCentralParticlePosition = %" PSYM " %" PSYM " %" PSYM,
                  TestGravityCentralParticlePosition,
                  TestGravityCentralParticlePosition + 1,
                  TestGravityCentralParticlePosition + 2);

    /* if the line is suspicious, issue a warning */

    if (ret == 0 && strstr(line, "=") && strstr(line, "TestGravity") && line[0] != '#')
  ...
  for (dim = 0; dim < MetaData.TopGridRank; dim++)
    NumberOfSubgridZones[dim] = nint((TestGravitySubgridRight - TestGravitySubgridLeft) /
                                     ((DomainRightEdge[dim] - DomainLeftEdge[dim]) / float(MetaData.TopGridDims[dim]))) *
                                RefineBy;
  ...
  if (NumberOfSubgridZones[0] > 0)
  {
    ...
    if (Subgrid->GridData->TestGravityInitializeGrid(TestGravityDensity * POW(float(RefineBy), MetaData.TopGridRank),
                                                     0, TestGravityCentralParticlePosition,
                                                     TestGravityUseBaryons) == FAIL)
    {
      ENZO_FAIL("Error in TestGravityInitializeGrid.\n");
    }
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR)
  {
    ...
    fprintf(Outfptr, "TestGravityUseBaryons        = %" ISYM "\n\n",
            TestGravityUseBaryons);
    fprintf(Outfptr, "TestGravityCentralParticlePosition        = % " ISYM "\n\n",
            TestGravityUseBaryons);
    fprintf(Outfptr, "TestGravityCentralParticlePosition =  % " GOUTSYM " %" GOUTSYM" %" GOUTSYM"\n\n",
            TestGravityCentralParticlePosition[0],
            TestGravityCentralParticlePosition[1],
            TestGravityCentralParticlePosition[2]);
  }
